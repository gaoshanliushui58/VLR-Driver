...
def chunk_arrays(arrays, m): # arrays: [acceleration, angular_velocity, forward_vector, right_vector, location, rotation]
    chunks = [chunk_array(arr, m) for arr in arrays]
    return chunks # [6][len/m][m][3]

def chunk_array(arr, m):
    chunks = [arr[i * m:(i + 1) * m] for i in range((len(arr) + m - 1) // m)]
    return chunks # 每m个为一组，是一个列表，共(len(arr) + m - 1) // m 组。m=20
...
def seg_compute_comfort_metric(acceleration,
    ...
    if episode_len <= per_step: # 帧数 < m，直接计算metric
        res = compute_comfort_metric( acceleration, angular_velocity, forward_vector, right_vector, location, rotation)
        return 1. if res else 0.
    seg_data = chunk_arrays([acceleration, angular_velocity, forward_vector, right_vector, location, rotation], per_step) # 每m个为一组，分组
    
    res = []
    for index in range(len(seg_data[0])): # len/m 组，例如1855帧，93组-1舍弃
        if len(seg_data[0][index]) < per_step: # 舍弃最后一组
            continue
    ...
    return res.count(True) / len(res) # len/m 组，True占总组数的比例
    pass
...
def compute_comfort_metric(
    ...
    res = np.array([ # 判断各种acc或jerk是否超过最大最小边界，如超过，则False
        _within_bound(
...
def _within_bound(
    ...
    return np.all(metric_within_bound, axis=-1) # 是否在 最大最小 阈值内
...
def read_from_json(filepath, metric_dir=None):
    ...
    for record in data["_checkpoint"]["records"]:
        ...
        with open(filepath, 'r') as file:
            ...
            for k, v in json_data.items(): # 读入metric.json中的数值
        ...
        for k, v in temp_dict.items():
            temp_dict[k] = np.array(v) # 列表转nparray
        all_data.append(temp_dict)
        if len(record["infractions"]["min_speed_infractions"]) < 1:
            continue
        else:
            driving_e = []
            for min_speed_infrac in record["infractions"]["min_speed_infractions"]: # 小速度的值
                number = re.search(r"\b\d+\.?\d*%", min_speed_infrac)
                if float(number.group().rstrip('%')) > 1000:
                    continue
                driving_e.append(float(number.group().rstrip('%')))
            driving_e = sum(driving_e) / len(driving_e) # 求平均
            driving_efficiency.append(driving_e)
    return all_data, driving_efficiency

if __name__=='__main__':
    argparser = argparse.ArgumentParser(description=__doc__)
    argparser.add_argument('-f', '--file', default="/data01/kfj_data01/HOME/Bench2Drive/output/tmp0925_5route_01/merged.json", help='route file')
    argparser.add_argument('-m', '--metric_dir', default="/data01/kfj_data01/HOME/Bench2Drive/output/tmp0925_5route_01")
    args = argparser.parse_args()
    all_data, driving_efficiency_list = read_from_json(args.file, args.metric_dir) # all_data：所有路线的acc、angvel、forwvec、rightvec、loc、rot等；driving_efficiency_list：驾驶效率，小速度的平均值
    comfort_res = []
    for record in all_data:
        comfort_res.append(seg_compute_comfort_metric(**record)) # 计算每条路线的舒适值
    print(f'Driving Efficiency={sum(driving_efficiency_list) / len(driving_efficiency_list)}') # 驾驶效率，车速是最小速度的%多少
    print(f'Driving Smoothness={sum(comfort_res)/len(comfort_res)}') # 舒适值，acc或jerk没有过大过小