...
import sys
# sys.path.append('/data01/kfj_data01/HOME/Bench2Drive/leaderboard/team_code/tcp_admlp')
import carla
import signal
...
class LeaderboardEvaluator(object):
    ...
    def __init__(self, args, statistics_manager):
        ...
        print("module_name:", module_name) #### tcp_b2d_agent
        sys.path.insert(0, os.path.dirname(args.agent))
        self.module_agent = importlib.import_module(module_name)
        print("self.module_agent:", self.module_agent)
    ...
    def _setup_simulation(self, args):
        ...
        cmd1 = f"{os.path.join(self.carla_path, 'CarlaUE4.sh')} -RenderOffScreen -nosound -carla-rpc-port={args.port} -graphicsadapter={args.carla_gpu_rank}"
        # cmd1 = f"{os.path.join(self.carla_path, 'CarlaUE4.sh')} -carla-rpc-port={args.port} -graphicsadapter={args.gpu_rank}"
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        try:
            ...
            self.manager.run_scenario()  ###运行模型
    ...
    def run(self, args):
        ...
        if crashed:
            cmd2 = "ps -ef | grep '-graphicsadapter="+ str(args.carla_gpu_rank) + "' | grep -v grep | awk '{print $2}' | xargs -r kill -9"
...
def main():
    ...
    parser.add_argument("--carla-gpu-rank", type=int, default=0)
