...
class StatisticsManager(object):
    ...
    def add_file_records(self, endpoint):
        """Reads a file and saves its records onto the statistics manager 读取文件，获取其记录，并将其添加到统计管理器的检查点记录中"""
    ...
    def sort_records(self):
        """Sorts the route records according to their route id (This being i.e RouteScenario0_rep0) 按路线ID和重复号（rep）对路线记录进行排序，确保访问记录时顺序正确。"""
    ...
    def write_live_results(self, index, ego_speed, ego_control, ego_location):
        """Writes live results 将实时统计信息(如route  ID、场景名称、分数和违规)写入调试端点文件。还记录了最近五次违规。"""
